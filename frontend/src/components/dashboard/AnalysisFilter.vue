  ...
  <section class="border-right height-full">
    <div>
      ...
      <div class="select-bar">
        <ul>
          <li v-for="main in analysis" :key="main.value" class="main-menu">
            <!-- value값 여부에 따라 class부여 -->
            <button
              class="main-button"
              :class="
                getMainBtnActive(main.value)
                  ? 'main-button-active'
            ...
            <!-- value값 있는 경우 -->
            <ul v-if="selected.indexOf(main.value) >= 0">
              <li
              ...
              >
                <button
                  class="sub-button"
                  :class="
                    getSubBtnActive(main.value, sub.value)
                      ? 'sub-button-active'
                      : 'sub-button-inactive'
                  "
    ...
    <div>
      <b-button class="common-btn reset-btn" @click="reset"> 초기화 </b-button>
    </div>

    <div v-show="showDownloadBtn">
      <b-button class="common-btn download-btn" @click="download">
        선택 데이터 다운<br />
...
export default class AnalysisFilter extends Vue {
  private chartOn!: boolean;
  private showDownloadBtn: boolean;
  ...
  private getSubBtnActive(main: string, sub: number) {
    if (
      this.selected.indexOf(main) === 1 &&
      this.secondOptions.indexOf(sub) >= 0
    )
      return true; // 두번째 선택된 분석변수 와 두번째 서브메뉴가 값이 있을경우 true 반환
    else if (
      this.selected.indexOf(main) === 0 &&
      this.firstOptions.indexOf(sub) >= 0
    )
      return true; // 첫번째 선택된 분석변수와 첫번째 서브메뉴가 값이 있을경우 true 반환
    else return false;
  }
  ...
  private change() {
    ...
    if (existsPro) {
      if (this.selected[0] === 'PRODUCT' && this.selected.length > 1) {
        // 2개선택 후 product 선택하는 경우? 선택되는것을 방지하는 차원
